...
all_df1 = pd.read_csv(os.getenv("RATINGS_PATH", "modeling/female_train.csv"))
all_df1 = all_df1[['user_id', 'movie', 'rating']]
all_df1 = all_df1.rename(columns={"user_id": "user"})
all_df2 = pd.read_csv(os.getenv("RATINGS_PATH", "modeling/male_train.csv"))
all_df2 = all_df2[['user_id', 'movie', 'rating']]
all_df2 = all_df2.rename(columns={"user_id": "user"})
frames = [all_df1, all_df2]
...
#SVD_MODEL_DIR = os.path.abspath(os.getenv("MODEL_DUMP_PATH", "modeling/SVD_model_2.0.dump"))
#_, user_algo2 = dump.load(SVD_MODEL_DIR)

SVD_MODEL_DIR = os.path.abspath(os.getenv("MODEL_DUMP_PATH", "modeling/SVD_female.dump"))
_, user_algo2 = dump.load(SVD_MODEL_DIR)

SVD_MODEL_DIR2 = os.path.abspath(os.getenv("MODEL_DUMP_PATH", "modeling/SVD_male.dump"))
_, user_algo3 = dump.load(SVD_MODEL_DIR)
...
def recommend(user_id):
    ...
    recommendString, gender = SVD_model(user_id_int)

    log_recommendation(user_id_int, recommendString, gender)
...
def SVD_model(user_id_int):
    ...
    # Check gender to send to appropriate model, need to load separate models

    gender = get_gender(user_id_int)

    if gender == 'M':
        print("Male User Detected!")
        ratings = np.array([user_algo3.predict(test_user,movie_encoded2movie[i]).est for i in movies_not_watched])
    else:
        print("Female User Detected!")
    ...
    return recommendString, gender
    
def get_gender(user_id_int):
    conn = get_db_connection()
    
    cur = conn.cursor()
    cur = conn.cursor()
    cur.execute(f'SELECT * FROM users_table WHERE user_id = {user_id_int};')
    users = cur.fetchall()
    cur.close()
    conn.close()
...
def get_db_connection():
    conn = psycopg2.connect(host='sqlalchemy-orm-psql',
                            database='sqlalchemy',
                            user=os.environ['DB_USERNAME'],
                            password=os.environ['DB_PASSWORD'])
    return conn

def log_recommendation(user_id, recommendation, gender):

    conn = get_db_connection()
    cur = conn.cursor()

    elements = "user_id, model_id, data_id, pipeline_id, recommendation, time_stamp"
    ...
    if gender == 'M':
        model = os.environ['MALE_MODEL_ID']
        data = os.environ['MALE_DATA_ID']
    else: 
        model = os.environ['FEMALE_MODEL_ID']
        data = os.environ['FEMALE_DATA_ID']
    
    log = f'\'{user_id}\', \'{model}\', \'{data}\', \'{pipeline}\', \'{recommendation}\', \'{stamp}\''

    cur.execute(f'INSERT INTO recommendations_with_pipeline({elements}) VALUES ({log})')
